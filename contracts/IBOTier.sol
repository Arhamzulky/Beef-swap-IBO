...
contract IBOTier is Ownable, IIBOTier {
    IIBOPoint public iboPointer;
    ...
    constructor() {
        ...
        tiers.push(Tier({ tierId: 4, minPoint: 1000 ether, multiplier: 10 })); // gold
    }
    ...
    function setIBOPointer(IIBOPoint _iboPointer) external onlyOwner {
        require(address(_iboPointer) != address(0), "Invalid address");

        iboPointer = _iboPointer;
    }

    function getTier(address user) external view override returns (uint256) {
        uint256 point = iboPointer.getPoint(user);
